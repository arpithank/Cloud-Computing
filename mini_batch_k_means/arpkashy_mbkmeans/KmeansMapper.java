...
public class KmeansMapper extends CollectiveMapper<String, String, Object, Object> {
	...
	protected void setup(Context context) throws IOException, InterruptedException {
		...
		bSize = configuration.getInt(KMeansConstants.BATCH_SIZE, 10);
	...
	protected void mapCollective( KeyValReader reader, Context context) throws IOException, InterruptedException {
	 ...
	 private void computation(Table<DoubleArray> cenTable, Table<DoubleArray> previousCenTable,ArrayList<DoubleArray> dataPoints){
	  ...
	  private void runKmeans(List<String> fileNames, Configuration conf, Context context) throws IOException {
		  // -----------------------------------------------------
		  ArrayList<DoubleArray> bPoints = null;
		  // Load centroids
	...
	//generation of mini batch from dataPoints randomly
	private ArrayList<DoubleArray> generateBatchPoints(ArrayList<DoubleArray> dataPoints) {
		ArrayList<DoubleArray> batch = new ArrayList<>();
		Random rand = new Random();
		int point = -1;

		//points added to the batch from the dataPoints in random
		for(int i = 0; i < bSize; i++) {
			point = rand.nextInt(dataPoints.size());
			batch.add(dataPoints.get(point));
		}
